...
The =plug#begin()= function sets up the plugin directory (this configuration uses the default directory: =~/.vim/plugged=) and the =plug#end()= function initializes the plugin system.
